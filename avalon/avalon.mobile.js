/*==================================================
 ...
 avalon.mobile.js 1.43 built in 2015.5.14
...
function scanTag(elem, vmodels, node) {
    ...
    if (typeof a === "string") {
        ...
        avalon(elem).removeClass(node.name) //[aslin:]华为C8650 Android2.3.3 不支持elem.classList
    ...
    avalon.fn[method + "Class"] = function(cls) {
        ...
        if (cls && typeof cls === "string" && el && el.nodeType === 1) {
            cls.replace(/\S+/g, function(c) {
                fakeClassList(el)[method](c)// [aslin:]华为C8650 Android2.3.3 不支持elem.classList
            })
        }
        return this
    }
...
avalon.fn.mix({
    hasClass: function(cls) {
        ...
        return el.nodeType === 1 && fakeClassList(el).contains(cls) // [aslin:]华为C8650 Android2.3.3 不支持elem.classList
    },
...
new function() {// jshint ignore:line
    ...
    function touchmove(event) {
        ...
        /*
            ...
            [aslin:]在lenovo K900/Android 4.2.2 页面滚动不流畅，先将阈值改为30；但是在GN135/Android 4.0.4下，touchend不触发
            lenovo机器会重复触发touchmove事件
            这两种机型完全矛盾
            判断用户手势，如果是水平滑动则阻止默认行为
        */
        if (avalon.mvCount == 0 && isAndroid && Math.abs(touchProxy.x - e.x) > Math.abs(touchProxy.y - e.y)) {
            event.preventDefault()
        }
        avalon.mvCount++;
        if (_isPointerType && !isPrimaryTouch(event)) return
